...
st.caption("NEWS2 Engine + Multi-Agent Debate + Cryptographic Provenance")
...
with tab1:
    ...
    with col1:
        ...
        narrative = st.text_area("Clinical Presentation", "Chest pain, shortness of breath.")
        ...
        if st.button("Run Triage Pipeline", type="primary", use_container_width=True):
            try:
                ...
                st.error(f"Security violation: {e}")
                st.stop()
            vitals = VitalsInput(
                respiration_rate=rr,
    ...
    with col2:
        ...
        if "result" in st.session_state:
            ...
            st.divider()
            st.subheader("Human Sign-Off Required")
            st.warning("PENDING_CLINICIAN_REVIEW — No bed allocated until approved")
            ca, cb2 = st.columns(2)
            with ca:
                if st.button("Clinician Approved", type="primary"):
                    st.success("Bed " + final.get("allocated_bed_id","—") + " allocated.")
                    st.balloons()
            with cb2:
                if st.button("Reject — Manual Review"):
                    st.error("Rejected.")
        else:
            st.info("Fill in patient details and click Run Triage Pipeline.")

with tab2:
    st.header("📊 Clinical Validation Results")
    st.caption("918 real patients — Kaggle Heart Failure Dataset (ODbL-1.0)")
    st.success("🏆 CLINICAL GRADE — AUROC 0.9356")
    ...
    with cg:
        ...
        st.success("Bias Gap: 0.0115 — OK (under 0.1 threshold)")
    with ca2:
        st.markdown("**Age Bias Check**")
        st.metric("Elderly 65+ AUROC", "0.9238")
        st.metric("Younger AUROC", "0.9362")
        st.success("Bias Gap: 0.0124 — OK (under 0.1 threshold)")
    st.divider()
...
| Model | Logistic Regression on NEWS2 features |
| Evaluation | Full cohort — no train/test leakage |
    """)
    st.divider()
    st.subheader("Why This Matters")
    st.markdown("""
- **AUROC 0.9356** — system correctly ranks 93.56% of critical vs non-critical pairs
- **Sensitivity 90.4%** — only 9.6% of critical patients are missed
- **Bias gaps under 0.013** — equal performance across gender and age groups
- **Brier Score 0.0954** — probability calibration is clinically reliable
    """)

with tab3:
    st.header("System Architecture")
    st.markdown("**Live:** https://sentinel-cdss-production.up.railway.app/")
    st.markdown("**GitHub:** https://github.com/ishantgupta30/sentinel-cdss")
    st.markdown("""
### Tech Stack
...
| Standard | RCP NEWS2 2017 |

### Safety Features
- PHI scrubbing before every LLM call
- Prompt injection detection
- Human sign-off before bed allocation
- AUROC 0.9356 on 918 real patients
    """)